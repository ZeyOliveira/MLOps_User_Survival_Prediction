...
def load_to_sql(file_path):
    ...
    engine = sqlalchemy.create_engine(f"postgresql+psycopg2://{conn.login}:{conn.password}@user-survival-prediction_b58149-postgres-1:{conn.port}/{conn.schema}")
...
with DAG(
    ...
    # Extract data from GCS steps
    list_files = GCSListObjectsOperator(
        task_id="list_files",
        bucket="mlops-bucket-03", 
    )
    ...
    # Transform and Load data to SQL
